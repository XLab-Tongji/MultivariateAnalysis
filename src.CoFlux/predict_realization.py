...
def holt_winter_predict(df, column_name, period, smoothing_level=0.6, smoothing_trend=0.6,
...
# 差分 选择oneDayLength 或者oneDayLength*7
def diff_predict(df, column_name, length):
    result = []
...
# 历史平均 选择宽度为1，2，3，4*7*oneDayLength
def his_average_predict(df, column_name, length):
    result = []
    df_x = df[column_name]
...
# 历史中值 选择宽度为1，2，3，4*7*oneDayLength
def his_median_predict(df, column_name, length):
    result = []
    df_x = df[column_name]
    # 预测值 选择窗口为window
...
def get_af_set(df, column_name, period, oneDayLength):
    result = []
    # 由于tsd最后period/2个数字是nan 所以其他的都要删除最后period/2对齐
    # 两种diff 1 day 1 week，尾对齐长度
    print("now diff")
    minLength = len(df)
...
def get_result(df, column_name_x, column_name_y, period=24, oneDayLength=24):
    """
    :param period: 周期，一般与oneDayLength相等
    :param oneDayLength: 一天中的采集的数据点数量
    :return:
    """
    afx_set = get_af_set(df=df, column_name=column_name_x, period=period, oneDayLength=oneDayLength)
    afy_set = get_af_set(df=df, column_name=column_name_y, period=period, oneDayLength=oneDayLength)
